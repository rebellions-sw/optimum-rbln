...
def register_rbln_custom_paged_attention():
    ...
    def paged_attn_decode_cpu(q, k, v, mask, kcache, vcache, seq, scale, block_table, block_size):
        return q

    @register_fake("rbln_custom_ops::paged_attn_decode")
    def paged_attn_decode_abstract(q, k, v, m, kcache, vcache, seq, scale, block_table, block_size):
        return q
    ...
    def paged_attn_prefill_cpu(q, k, v, mask, kcache, vcache, seq, scale, block_table, block_size):
        return q

    @register_fake("rbln_custom_ops::paged_attn_prefill")
    def paged_attn_prefill_abstract(q, k, v, m, kcache, vcache, seq, scale, block_table, block_size):
        return q
...
def register_rbln_custom_paged_causal_attention():
    ...
    def paged_causal_attn_decode_cpu(q, k, v, kcache, vcache, seq, scale, block_table, block_size):
        return q

    @register_fake("rbln_custom_ops::paged_causal_attn_decode")
    def paged_causal_attn_decode_abstract(q, k, v, kcache, vcache, seq, scale, block_table, block_size):
        return q
    ...
    def paged_causal_attn_prefill_cpu(q, k, v, kcache, vcache, seq, scale, block_table, block_size):
        return q

    @register_fake("rbln_custom_ops::paged_causal_attn_prefill")
    def paged_causal_attn_prefill_abstract(q, k, v, kcache, vcache, seq, scale, block_table, block_size):
        return q
