...
from transformers import AutoModel, AutoModelForCausalLM, AutoConfig, PretrainedConfig, PreTrainedModel
...
class RBLNDecoderOnlyModel(RBLNModel, RBLNDecoderOnlyFlashAttentionMixin):
    ...
    def get_pytorch_model(
        cls, model_id: str, *args, rbln_config: Optional[RBLNDecoderOnlyModelConfig] = None, num_hidden_layers: Optional[int] = None, **kwargs
    ) -> PreTrainedModel:
        if rbln_config and rbln_config.quantization:
            model = cls.get_quantized_model(*args, rbln_config=rbln_config, **kwargs)
        else:
            if num_hidden_layers is not None:
                config, kwargs = AutoConfig.from_pretrained(model_id, return_unused_kwargs=True, num_hidden_layers=num_hidden_layers, **kwargs)
