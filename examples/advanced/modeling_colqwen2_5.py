...
class RBLNColQwen2_5ForRetrieval(RBLNQwen2_5_VLForConditionalGeneration):
    ...
    def get_compiled_model(
        cls, model: "PreTrainedModel", rbln_config: RBLNColQwen2_5ForRetrievalConfig
    ):
        ...
        meta_tensor_names = [
            name for name, _, _ in prefill_compile_config.input_info if "past_key_values" in name
        ]
        prefill_example_inputs = prefill_compile_config.get_dummy_inputs(
        ...
        compiled_prefill = compile_model(
            wrapped_model, prefill_compile_config, prefill_example_inputs, context
        )
    ...
    def _update_rbln_config(
        ...
        rbln_config.attn_impl, rbln_config.kvcache_partition_len, rbln_config.kvcache_block_size = (
            set_default_values(
        ...
        prefill_compile_config = RBLNCompileConfig(
            compiled_model_name="prefill", input_info=prefill_input_info
        )
    ...
    def _preprocess_chunked_prefill(self, inputs_embeds, attention_mask, position_embed):
        ...
        inputs_embeds = (
            inputs_embeds[:, attention_mask.bool()] if attention_mask is not None else inputs_embeds
        )
        position_embed = (
