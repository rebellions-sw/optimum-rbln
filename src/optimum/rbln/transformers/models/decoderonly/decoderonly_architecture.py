...
class AttentionOp(nn.Module):
    ...
    def forward(
        ...
        if self.phase == "decode":
            if self.use_attention_mask:
                attn_output = torch.ops.rbln_custom_ops.masked_attn_decode(
                    query_state,
                    key_state,
                    value_state,
                    attn_mask,
                ...
                attn_output = torch.ops.rbln_custom_ops.causal_masked_attn_decode(
                    query_state,
                    key_state,
                    value_state,
                    past_key_state.unsqueeze(2),
            ...
            if self.use_attention_mask:
                attn_output = torch.ops.rbln_custom_ops.masked_attn_prefill(
                    query_state,
                    key_state,
                    value_state,
                    attn_mask,
                ...
                attn_output = torch.ops.rbln_custom_ops.causal_masked_attn_prefill(
                    query_state,
                    key_state,
                    value_state,
                    past_key_state.unsqueeze(2),
...
class FlashAttentionOp(AttentionOp):
    ...
    def forward(
        ...
        if self.phase == "decode":
            if self.use_attention_mask:
                attn_output = torch.ops.rbln_custom_ops.flash_masked_attn_decode(
                    query_state,
                    key_state,
                    value_state,
                    attn_mask,
                ...
                attn_output = torch.ops.rbln_custom_ops.flash_causal_masked_attn_decode(
                    query_state,
                    key_state,
                    value_state,
                    past_key_state.unsqueeze(2),
            ...
            if self.use_attention_mask:
                attn_output = torch.ops.rbln_custom_ops.flash_masked_attn_prefill(
                    query_state,
                    key_state,
                    value_state,
                    attn_mask,
                ...
                attn_output = torch.ops.rbln_custom_ops.flash_causal_masked_attn_prefill(
                    query_state,
                    key_state,
                    value_state,
                    past_key_state.unsqueeze(2),
